...
def read_grid() -> Grid:
    ...
    for x in range(len(grid[0])):
        for y in range(len(grid)):
            ...
            if grid[y][x] == 'a':
                starting_positions.append(Vector(x=x, y=y))
            grid[y][x] = ord(grid[y][x]) - ord('a')

    return Grid(grid=grid, starting_positions=starting_positions, finishing_position=finishing_position)


def find_min_distance_path(grid: Grid):
    candidates = set(grid.starting_positions)
    visited = set()
    distance = 0
    while True:
        distance += 1
